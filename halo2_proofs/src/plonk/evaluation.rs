            ...
            if use_optimized {
                ...
                let lookup_cosets: Vec<_> = lookups
                    ...
                    .map(|lookup| {
                        ...
                        let permuted_input_coset = pk.vk.domain.coeff_to_extended(&lookup.permuted_input_poly);
                        let permuted_table_coset = pk.vk.domain.coeff_to_extended(&lookup.permuted_table_poly);
                        (product_coset, permuted_input_coset, permuted_table_coset)
                    })
                    .collect();
                
                log::debug!("    Batch polynomial conversion: {:?}", eval_start.elapsed());
                
                // 2. APPLY CONSTRAINTS: Process each lookup sequentially but with pre-computed cosets
                for (n, _lookup) in lookups.iter().enumerate() {
                    let (product_coset, permuted_input_coset, permuted_table_coset) = &lookup_cosets[n];
                    
                    // Apply constraints for this specific lookup using parallelize
                    parallelize(&mut values, |values, start| {
